...
import { CLEAR_NOTIFICATIONS, HIDE_NOTIFICATION, SHOW_NOTIFICATION } from "./actionTypes";
import {
    clearNotifications,
    hideNotification,
    showNotification,
...
import { NOTIFICATION_TIMEOUT_TYPE, RAISE_HAND_NOTIFICATION_ID } from "./constants";
...
MiddlewareRegistry.register((store) => (next) => (action) => {
    ...
    switch (action.type) {
        ...
        case PARTICIPANT_LEFT: {
            ...
            const leaveClassroomState = isLeaveClassroom(currentState);

            // 디버깅용 로그
            console.log("isLeaveClassroom state:", leaveClassroomState);
            console.log("toolbox state:", currentState["features/toolbox"]);

            if (!joinLeaveNotificationsDisabled() && !leaveClassroomState) {
                const participant = getParticipantById(currentState, action.participant.id);
                ...
                if (
                    ...
                    dispatch(showParticipantLeftNotification(getParticipantDisplayName(currentState, participant.id)));
                }
            }
