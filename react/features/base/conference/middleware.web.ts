...
import { setPrejoinPageVisibility, setSkipPrejoinOnReload } from "../../prejoin/actions.web";
...
MiddlewareRegistry.register((store) => (next) => (action) => {
    ...
    switch (action.type) {
        ...
        case CONNECTION_ESTABLISHED: {
            ...
            logger.debug(
                `On connection established: prejoinVisible: ${prejoinVisible}, initialGUMPromiseExists=${Boolean(
                    initialGUMPromise
                ...
                promise.then(() => {
                    ...
                    for (const track of localTracks) {
                        ...
                        if (
                            ...
                            trackReplacePromises.push(
                                dispatch(replaceLocalTrack(track.jitsiTrack, null)).catch((error: any) => {
                                    logger.error(
                                        `Failed to replace local track (${track.jitsiTrack}) with null: ${error}`
                                    );
                                })
                            );
                        }
