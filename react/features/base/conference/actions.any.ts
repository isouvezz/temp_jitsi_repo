...
function _addConferenceListeners(conference: IJitsiConference, dispatch: IStore["dispatch"], state: IReduxState) {
    ...
    conference.on(JitsiConferenceEvents.CONFERENCE_UNIQUE_ID_SET, (..._args: any[]) =>
        dispatch(conferenceUniqueIdSet(conference))
    );
    ...
    conference.on(JitsiConferenceEvents.PROPERTIES_CHANGED, (properties: Object) =>
        dispatch(conferencePropertiesChanged(properties))
    );
