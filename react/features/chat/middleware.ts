...
import { ADD_MESSAGE, CLOSE_CHAT, OPEN_CHAT, SEND_MESSAGE, SEND_REACTION, SET_FOCUSED_TAB } from "./actionTypes";
    ...
    switch (action.type) {
        ...
        case OPEN_CHAT: {
            ...
            if (focusedTab === ChatTabs.CHAT) {
                ...
                if (isSendGroupChatDisabled(store.getState()) && privateMessageRecipient && !action.participant) {
...
function _handleReceivedMessage(
    ...
    // 알림 표시 비활성화 - 알림이 쌓이지 않도록 함
    const shouldShowNotification =
        userSelectedNotifications?.["notify.chatMessages"] && !hasRead && !isReaction && (!timestamp || lobbyChat);
    ...
    // 알림 표시 활성화 (최대 1개만 표시되도록 reducer에서 처리)
