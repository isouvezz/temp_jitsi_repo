...
import { _openPasswordRequiredPrompt } from "../room-lock/actions";

import { CANCEL_LOGIN, LOGIN, LOGOUT, STOP_WAIT_FOR_OWNER, UPGRADE_ROLE_FINISHED, WAIT_FOR_OWNER } from "./actionTypes";
import {
    hideLoginDialog,
    openLoginDialog,
    openTokenAuthUrl,
...
MiddlewareRegistry.register((store) => (next) => (action) => {
    switch (action.type) {
        ...
        case CONFERENCE_FAILED: {
            ...
            if (
                error.name === JitsiConferenceErrors.AUTHENTICATION_REQUIRED ||
        ...
        case CONFERENCE_JOINED: {
            ...
            if (isTokenAuthEnabled(config) && config.tokenAuthUrlAutoRedirect && state["features/base/jwt"].jwt) {
