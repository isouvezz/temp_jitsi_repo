...
import { CONFERENCE_LEFT, CONFERENCE_WILL_LEAVE } from "../base/conference/actionTypes";
import { setLeaveClassroom } from "./actions.any";
...
MiddlewareRegistry.register((store) => (next) => (action) => {
    switch (action.type) {
        ...
        case CONFERENCE_LEFT: {
            ...
            console.log("CONFERENCE_LEFT triggered - setting isLeaveClassroom to true");
        ...
        case CONFERENCE_WILL_LEAVE: {
            ...
            console.log("CONFERENCE_WILL_LEAVE triggered - setting isLeaveClassroom to true");
            dispatch(setLeaveClassroom(true));
            ...
            if (action.type !== I_AM_VISITOR_MODE) {
                ...
                batch(() => {
                    ...
                    dispatch({
                        ...
                        participantMenuButtonsWithNotifyClick: _buildButtonsArray(
                            participantMenuButtonsWithNotifyClick,
                            customParticipantMenuButtons
                        ),
                    });
...
function _buildButtonsArray(
    ...
    const customButtonsWithNotifyClick =
        customButtons?.map(({ id }) => [id, NOTIFY_CLICK_MODE.ONLY_NOTIFY] as [string, NOTIFY_CLICK_MODE]) ?? [];
    const buttons = (Array.isArray(buttonsWithNotifyClick) ? buttonsWithNotifyClick : [])
        .filter(
            (button) => typeof button === "string" || (typeof button === "object" && typeof button.key === "string")
        )
        .map((button) => {
