...
const ParticipantContextMenu = ({
    ...
    if (_isModerator) {
        if (isModerationSupported) {
            if (
                _isAudioMuted &&
                !participant.isSilent &&
                !(isClickedFromParticipantPane && quickActionButtonType === QUICK_ACTION_BUTTON.ASK_TO_UNMUTE)
            ) {
                buttons.push(
                    <AskToUnmuteButton {...getButtonProps(BUTTONS.ASK_UNMUTE)} buttonType={MEDIA_TYPE.AUDIO} />
                );
            }
            if (
                _isVideoForceMuted &&
                ...
                buttons.push(
                    <AskToUnmuteButton {...getButtonProps(BUTTONS.ALLOW_VIDEO)} buttonType={MEDIA_TYPE.VIDEO} />
                );
            }
        }
    ...
    return (
        ...
        >
                    ...
                    actions={[
                        {
                            ...
                            customIcon: <Avatar participantId={drawerParticipant.participantID} size={20} />,
            ...
            {showVolumeSlider && (
                <ContextMenuItemGroup>
                    <VolumeSlider initialValue={_volume} key="volume-slider" onChange={_onVolumeChange} />
                </ContextMenuItemGroup>
            )}
