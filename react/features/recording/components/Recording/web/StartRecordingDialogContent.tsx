...
import { DROPBOX_LOGO, ICON_CLOUD, ICON_INFO, ICON_USERS, LOCAL_RECORDING } from "../styles.web";
...
class StartRecordingDialogContent extends AbstractStartRecordingDialogContent {
    ...
    _renderAdvancedOptions() {
        ...
        return (
            <>
                ...
                <div className="recording-header" onClick={this._onToggleShowOptions}>
                    <label className="recording-title-no-space">{t("recording.showAdvancedOptions")}</label>
                    <Icon
                ...
                {showAdvancedOptions && (
                    <>
                        <div className="recording-header space-top">
                            <label className="recording-title" htmlFor="recording-switch-transcription">
    ...
    _renderNoIntegrationsContent() {
        ...
        const { _localRecordingAvailable, integrationsEnabled, isValidating, isVpaas, selectedRecordingService, t } =
            this.props;
        ...
        return (
            <Container className={`recording-header ${jitsiRecordingHeaderClass}`} key="noIntegrationSetting">
                <Container className={contentRecordingClass}>
                    <Image alt="" className="content-recording-icon" src={ICON_CLOUD} />
                </Container>
                <label className="recording-title" htmlFor="recording-switch-jitsi">
                    {label}
                </label>
                {switchContent}
            </Container>
    ...
    _renderFileSharingContent() {
        ...
        return (
            <Container className="recording-header" key="fileSharingSetting">
                <Container className="recording-icon-container file-sharing-icon-container">
                    <Image alt="" className="recording-file-sharing-icon" src={ICON_USERS} />
                </Container>
                <label className="recording-title" htmlFor="recording-switch-share">
    ...
    _renderUploadToTheCloudInfo() {
        ...
        return (
            <Container className="recording-info" key="cloudUploadInfo">
                <Image alt="" className="recording-info-icon" src={ICON_INFO} />
                <Text className="recording-info-title">{t("recording.serviceDescriptionCloudInfo")}</Text>
            </Container>
        );
    }
        ...
        return (
            <Container>
                <Container className="logged-in-panel">
                    <Container>
                        <Text>{t("recording.loggedIn", { userName })}</Text>
                    </Container>
                    <Container>
                        <Text>
    ...
    _renderIntegrationsContent() {
        ...
        let labelContent = <Text className="recording-title">{t("recording.authDropboxText")}</Text>;
        ...
        if (fileRecordingsServiceEnabled || _localRecordingAvailable) {
            ...
            labelContent = (
                <label className="recording-title" htmlFor="recording-switch-integration">
            ...
            <Container>
                <Container
                    className={`recording-header ${
                        this._shouldRenderNoIntegrationsContent() ? "recording-header-line" : ""
                    }`}
                >
                    <Container className="recording-icon-container">
                        <Image alt="" className="recording-icon" src={DROPBOX_LOGO} />
                    </Container>
                    {labelContent}
                    {switchContent}
                </Container>
    ...
    _renderLocalRecordingContent() {
        ...
        return (
            <>
                <Container>
                    <Container className="recording-header recording-header-line">
                        <Container className="recording-icon-container">
                            <Image alt="" className="recording-icon" src={LOCAL_RECORDING} />
                        </Container>
                        <label className="recording-title" htmlFor="recording-switch-local">
                            {t("recording.saveLocalRecording")}
                        </label>
                        <Switch
                            checked={selectedRecordingService === RECORDING_TYPES.LOCAL}
                            className="recording-switch"
                            disabled={isValidating || !this.props.shouldRecordAudioAndVideo}
                ...
                {selectedRecordingService === RECORDING_TYPES.LOCAL && (
                    <>
                        {_localRecordingSelfEnabled && (
                            <Container>
                                <Container className="recording-header space-top">
                                    ...
                                    <label className="recording-title" htmlFor="recording-switch-myself">
                        ...
                        <Text className="local-recording-warning text">{t("recording.localRecordingWarning")}</Text>
